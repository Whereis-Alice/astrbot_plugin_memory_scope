...
趋势区间对比和最近活动见[专门说明](activity.md)。它们用于解释当时有哪些候选原因，而不是强行给每个插件分摊总内存。
...
- 对象扫描仍在 AstrBot 进程内，可能占 CPU、触发换入及停顿。对象普查始终按需；后端模式的自动诊断只运行轻量引用图扫描，默认每 15 分钟一次，约 1 秒 CPU 工作预算和 100000 个对象上限。
