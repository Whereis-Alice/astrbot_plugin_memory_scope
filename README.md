...
默认不开追踪意味着拿不到"哪个插件占了多少"，但**进程整体 RSS 趋势不需要 tracemalloc**。
v1.0.2 起，采样循环在追踪关闭时也照常记录一条只含 RSS / GC 的样本，所以装上插件什么都不改也能看到：

- 进程 RSS 随时间的曲线（判断"是不是一直在涨"这类问题，只要这条线就够）；
- GC 代计数、对象数、线程数等进程级指标；
- 重启后仍在的历史（样本持久化到插件 KV，每 5 次采样落盘一次）。

这类样本在报告里以 `history.traced_samples`（含归因的样本数）与 `history.samples`（总样本数）区分。
为了不让"追踪关闭"的样本污染按插件的曲线，插件系列图、增长趋势与基线差值都只取含归因的样本：
追踪中途开关不会在图上留下一段假的"掉到 0"。

想要覆盖导入期的完整数据，正确做法始终是 `PYTHONTRACEMALLOC`（见下一节）：它由解释器自己在启动时开启，不存在"中途插入"的放大效应。

### 运行时安装 vs 重启启动（v1.0.3）

`on_astrbot_loaded` 每个进程只派发一次，在 `CoreLifecycle.start()` 里，所以**在 Dashboard 里现装、启用或重载插件时这个钩子永远不会触发**。
v1.0.1 / v1.0.2 让采样循环无条件 `await` 这个事件，于是运行时安装的实例会永久卡住：没有采样、没有趋势、没有基线、没有告警、也不会持久化，页面只剩实时快照。

v1.0.3 把等待改成有上限（`LOADED_WAIT_TIMEOUT_SECONDS = 180`）：

- 正常重启启动：钩子在几十秒内到达，行为与之前完全一致（钩子 → 刷新插件表 → 可能开启追踪 → 8 s 后第一次采样）；
- 运行时安装：180 s 后超时，记一条 INFO 说明按运行时安装处理，刷新插件表后开始正常采样；
- **自动开启 tracemalloc 仍然只由钩子触发**。超时路径不会自动开追踪（此时无法确认导入风暴是否结束，宁可少一份归因也不要重演 36x），如果 `auto_start_tracemalloc=true` 会额外写一条 WARNING 提示用 `/mem trace on` 或重启。

超时期间并不是什么都没有：页面每次刷新（`GET plugins?sample=1`）本身就会记一条样本，所以新装用户不会看到空白趋势图。
